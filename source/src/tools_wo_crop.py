...
def read_images(paths, mode='RGB', verbose=True):
    ...
    for path in paths:
        try:
            with Image.open(path, 'r') as im:
                images.append(im.resize((crop_size,crop_size)).convert(mode).copy())
        except:
            if verbose:
                print('Failed to read {}'.format(path))
    return images
...
def get_generated_inception_stats(G, Z_sampler, inv_transform, size, batch_size=16):
    ...
    for i in tqdm(range(0, size, batch_size)):
        ...
        try:
            ...
            Z,_ =  next(Z_sampler)
            G_Z = G(Z.cuda().requires_grad_(True))
        ##
        ...
        ## lr
        if end - start < pred.shape[0]:
            pred_arr[start:end] = pred.cpu().data.numpy().reshape(pred.size(0), -1)[:end-start]
        else:
...
def get_statistics_of_dataloader(dataloader, num_samples = 50000, dims=2048, cuda=False, verbose=False):
    block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[dims]
    ...
    act = get_activations_for_dataloader(dataloader, model, num_samples, dims, cuda, verbose)
